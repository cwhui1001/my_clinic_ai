        ...
        <div className="mt-6 grid gap-5 lg:grid-cols-[1.4fr_0.8fr]">
          <div className="space-y-5">
            ...
            <Panel title="Point-in-time Patient Profile">
              {review.profileSnapshot.length ? <div className="grid gap-3 sm:grid-cols-2">{review.profileSnapshot.map((fact) => <div key={fact.revisionId} className="rounded-xl border border-slate-200 p-3"><p className="text-xs font-bold uppercase text-slate-500">{fact.kind.replaceAll("_", " ")} · {fact.status}</p><p className="mt-2 text-sm text-slate-800">{displayValue(fact.value)}</p><p className="mt-2 text-xs text-slate-400">Revision {fact.revisionId.slice(0, 8)}</p></div>)}</div> : <p className="text-sm text-slate-500">No structured facts existed at send time.</p>}
            </Panel>
