...
- Provider clients use direct server-only `fetch`, send only redacted message/context, and omit response bodies from errors: `src/server/openai/guest-response.ts` and `src/server/openai/patient-response.ts`. Gemini receives no email, phone, patient ID, guest token, or application safety identifier.
...
- Provider requests now use Gemini's native `generateContent` endpoint and JSON-schema output. Hosted API tier/data-use terms, key restrictions, contractual controls, and infrastructure logging still require deployment evidence; the repository does not claim Zero Data Retention.
...
Document the exact Gemini project, API tier, key restrictions, data-use controls, and contract; disable request-body capture in hosting/APM, centralize sanitized error reporting, and run a synthetic canary audit. Continue sending only locally redacted minimum context and do not claim healthcare compliance from schema validation alone.
...
- `src/config/server-env.ts:14-22,50-56` sets an explicit Gemini timeout, default 15 seconds and capped at 30 seconds.
...
If only Gemini hangs, Ana should see the safe failure response well before second 45. If the route/database hangs, the browser can spin indefinitely. If she writes `sakit dada`, the local emergency check executes but misses it, so the timeout response lacks the 999 floor.
...
- English `difficulty breathing` is High in `src/features/risk/policy.ts:13` and `src/features/guest-chat/policy.ts:7`, so the exact English message receives hard-coded 999 copy without calling Gemini.
...
Both Gemini calls share an explicit aborting provider timeout rather than relying on SDK/network defaults.
